// Generated by nitro
import type { Serialize, Simplify } from "nitropack/types";
declare module "nitropack/types" {
  type Awaited<T> = T extends PromiseLike<infer U> ? Awaited<U> : T
  interface InternalApi {
    '/api/rooms/:code': {
      'get': Simplify<Serialize<Awaited<ReturnType<typeof import('../../server/api/rooms/[code].get').default>>>>
    }
    '/api/rooms/create': {
      'post': Simplify<Serialize<Awaited<ReturnType<typeof import('../../server/api/rooms/create.post').default>>>>
    }
    '/api/rooms/join': {
      'post': Simplify<Serialize<Awaited<ReturnType<typeof import('../../server/api/rooms/join.post').default>>>>
    }
    '/api/rooms/list': {
      'get': Simplify<Serialize<Awaited<ReturnType<typeof import('../../server/api/rooms/list.get').default>>>>
    }
    '/__nuxt_error': {
      'default': Simplify<Serialize<Awaited<ReturnType<typeof import('../../node_modules/@nuxt/nitro-server/dist/runtime/handlers/renderer').default>>>>
    }
    '/__nuxt_island/**': {
      'default': Simplify<Serialize<Awaited<ReturnType<typeof import('../../server/#internal/nuxt/island-renderer').default>>>>
    }
  }
}
export {}